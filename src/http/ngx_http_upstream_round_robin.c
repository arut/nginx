...
/*
 * Peers in shared memory are used by every worker, so they are locked as
 * those of an upstream with a "zone" are.  A saved session comes from the
 * pool they are locked with, which the cleanup is for.
 */
