...
static ngx_int_t ngx_http_v3_proxy_non_buffered_copy_filter(void *data,
    ssize_t bytes);
static ngx_int_t ngx_http_v3_proxy_non_buffered_chunked_filter(void *data,
    ssize_t bytes);
...
ngx_http_v3_proxy_input_filter_init(void *data)
{
    ...
    ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "http3 proxy filter init s:%ui h:%d c:%d l:%O",
        ...
        || ctx->head)
    {
        ...
        u->pipe->length = 2; /* "0" LF LF */

        u->input_filter = ngx_http_v3_proxy_non_buffered_chunked_filter;
        u->length = 1;
    }
...
ngx_http_v3_proxy_body_filter(ngx_event_pipe_t *p, ngx_buf_t *buf)
{
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_AGAIN) {
            ...
            p->length = ctx->ctx.chunked.length;
...
free_buf:
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, p->log, 0,
                   "http3 proxy chunked state %ui, length %O",
                   ctx->ctx.chunked.state, p->length);
...
static ngx_int_t
ngx_http_v3_proxy_non_buffered_chunked_filter(void *data, ssize_t bytes)
{
    ngx_http_request_t   *r = data;
