...
/*
 * A module advertises in its flags the kinds of tenant it is eligible for,
 * and a parse of one holds in cf->dynamic what it needs of a module for its
 * directives to be allowed and its configurations to be made.  What the
 * bits are is up to the type of module; ngx_http_config.h has those of an
 * http module.
 */
#define ngx_module_tenant(m, type)  ((m)->flags & (type))
