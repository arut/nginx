...
ngx_ssl_handshake(ngx_connection_t *c)
{
    ...
    /* TODO dirty, ideally need a separate ctx */

    method = (c->type == SOCK_DGRAM) ? DTLS_method() : SSLv23_method();

    ctx = SSL_get_SSL_CTX(c->ssl->connection);

    if (SSL_CTX_get_ssl_method(ctx) != method) {
        SSL_CTX_set_ssl_version(ctx, method);
    }
#if 0
    if (SSL_get_ssl_method(c->ssl->connection) != method) {
        SSL_set_ssl_method(c->ssl->connection, method);
    }
#endif
