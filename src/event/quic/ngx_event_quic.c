...
static void ngx_quic_write_handler(ngx_event_t *rev);
...
ngx_quic_handshake(ngx_connection_t *c)
{
    ...
    if (c->buffer) {
        if (ngx_quic_handle_datagram(c, c->buffer) != NGX_OK) {
            return NGX_ERROR;
        }

    } else {
        if (ngx_quic_do_handshake(c) != NGX_OK) {
            return NGX_ERROR;
        }
    }
...
static void
ngx_quic_write_handler(ngx_event_t *rev)
{
    ngx_log_debug0(NGX_LOG_DEBUG_EVENT, rev->log, 0, "quic write handler");
}
...
ngx_quic_shutdown(ngx_connection_t *c)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, c->log, 0,
                   "quic close %s", qc->closing ? "resumed": "initiated");
    ...
    if (!qc->closing) {
        ...
        if (qc->error == NGX_QUIC_ERR_CLOSE) {
            ...
            ngx_log_debug5(NGX_LOG_DEBUG_EVENT, c->log, 0,
                           ...
                           "%serror:%ui \"%s\"",
                           c->ssl->no_wait_shutdown, qc->draining,
    ...
    if (c->ssl->no_wait_shutdown && qc->close.timer_set) {
        /* do not wait for timer in case of fatal error */
        ngx_del_timer(&qc->close);
    }

    /* TODO make ngx_quic_close_streams() respect c->ssl->no_wait_shutdown */
